...
lifecycle and evidence authorities. Scheduler-owned lease operations, complete run terminalization,
and ticket/run lifecycle creation and settlement now use asynchronous repository contracts with
active JSON and tested PostgreSQL implementations. PostgreSQL lifecycle bundles are transactional;
JSON remains a non-transactional development stage with startup reconciliation. The server has not
switched its runtime authority; see
