...
class JsonRunLeaseRepository {
  ...
  async listExpiredRunningRuns({ limit = 100 } = {}) {
    ...
    return this.readRuns()
      .filter(run => run.status === 'running' &&
        (!run.leaseOwner || !run.leaseExpiresAt || this._isExpired(run, clock.ms)))
      .sort((left, right) => {
  ...
  async startClaimedRun({ runId, leaseOwner, leaseDurationMs, eventPayload = {} }) {
    ...
    const clock = this._now();
    if (!run || run.status !== 'pending' || !this._hasLiveLease(run, owner, clock.ms)) return null;

    run.status = 'running';
    run.startedAt = run.startedAt || clock.iso;
    Object.assign(run, this._lease(run, owner, duration, clock));
    const callerPayload = typeof eventPayload === 'function'
      ? this._payload(eventPayload(run), 'eventPayload')
      : this._payload(eventPayload, 'eventPayload');
    this.writeRuns(runs);
    const event = await this.appendEvent({
      type: 'run.started',
      ticketId: run.ticketId,
  ...
  async recoverExpiredRun({ runId, eventType = 'run.resumed', eventPayload = {} }) {
    ...
    if (!run || run.status !== 'running' ||
        (run.leaseOwner && run.leaseExpiresAt && !this._isExpired(run, clock.ms))) return null;
