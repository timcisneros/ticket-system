...
// ── Test 5: Run-start context is prepared once per pending page ──
async function testRunStartContextPreparedOncePerPage() {
  const pendingRuns = [
    { id: 21, ticketId: 210, status: 'pending', agentId: 1, createdAt: '2026-01-01T00:00:00Z' },
    { id: 22, ticketId: 220, status: 'pending', agentId: 2, createdAt: '2026-01-01T00:00:01Z' }
  ];
  const sharedContext = { agentById: new Map([[1, { id: 1 }], [2, { id: 2 }]]) };
  ...
  const scheduler = createRuntimeScheduler({
    ...
    prepareRunStartContext: async runs => {
      prepareCalls += 1;
      assertEqual(runs.length, 2, 'context preparation should receive the bounded pending page');
      return sharedContext;
    },
    getRunStartBlockReason: async (_run, context) => {
      observedContexts.push(context);
      return 'local_model_concurrency_limit';
    },
    tryReserveRunStart: () => { throw new Error('blocked runs must not reserve capacity'); },
    acquireRunLease: () => { throw new Error('blocked runs must not acquire a lease'); },
    expireStaleRunLeases: () => {},
    isRunStarting: () => false,
    isRunActiveInMemory: () => false,
    runner: { startRun: () => { throw new Error('blocked runs must not start'); } }
  });

  await scheduler.tick();

  assertEqual(prepareCalls, 1, 'run-start context must be prepared once per scheduler page');
  assertEqual(observedContexts.length, 2, 'every pending candidate should use the prepared context');
  assert(observedContexts.every(context => context === sharedContext), 'all candidates must share the same tick-local context');

  console.log('  ✓ context-prepared-once: one bounded context is shared across the pending page');
}
...
async function main() {
  ...
  const tests = [
    ...
    testRunStartContextPreparedOncePerPage
  ];
