...
const inboxResolveSource = source.slice(
  source.indexOf("fastify.post('/api/inbox/threads/:id/resolve'"),
  source.indexOf("fastify.get('/api/logs'", source.indexOf("fastify.post('/api/inbox/threads/:id/resolve'"))
);
assert(
  /if \(thread\.kind === 'blocker'\) \{[\s\S]*runWithNewEventJournalAdmission\(/.test(inboxResolveSource),
  'inbox blocker resolution must reserve journal capacity before triage mutation'
);
assert(
  !/eventJournalAdmission:\s*true/.test(routeOptions('post', '/api/inbox/threads/:id/resolve')),
  'deliverable acknowledgement must not consume journal admission through the mixed inbox route'
);
