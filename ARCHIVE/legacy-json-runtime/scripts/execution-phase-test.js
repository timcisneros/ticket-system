...
const { isRunPhaseTransitionAllowed } = require('../persistence/json/run-phase-repository');
...
function testPhaseStateAdvancesForward() {
  const advance = (run, phase) => {
    if (isRunPhaseTransitionAllowed(run.currentPhase || 'planning', phase)) {
