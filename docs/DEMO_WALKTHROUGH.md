...
approval workflows, child-ticket execution, or any rerun-from-triage. `maxAttempts` is the
only enforced policy field, and only for **manual** rerun-from-start. Triage resolution is a
human acknowledgement that never changes completion or verification outcomes. See
