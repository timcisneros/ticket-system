...
| Event durability | Queued event writes could advance an in-memory chain before persistence, restart chains at zero, and leave final-event tampering undetectable. | Event append is synchronous and fail-closed; a run-chain tip advances only after the sealed event is appended. Restart restores one continuous per-run chain, hashes cover forensic metadata and payload, and every run and non-run event must satisfy the same strict envelope schema. |
| Run compatibility | Compatibility for old terminal event names, unsealed/reset chain segments, and missing run-limit snapshots added branches or reinterpreted old evidence using current settings. | Run evidence supports one schema (`schemaVersion: 1`) and one lifecycle (`run.execution_completed` then `run.terminalized`). Startup refuses old or unsealed events, and missing immutable run-limit snapshots are surfaced as unsupported rather than replaced with current defaults. |
...
The final deterministic release checkpoint passed **50/50 checks in 99.3 seconds**. No
