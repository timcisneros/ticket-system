...
## Governed No-Progress Refusal Has No End-to-End Scenario (recorded 2026-08-02)

**Status:** open — test-coverage gap, not a known defect.

The Tranche 5 mutation matrix removed the governed request gate's refusal:

```js
// persistence/postgres/store.js — prepareAndReserveNextGovernedRunRequest
if (!permitsGovernedRequest(evaluated.decision)) {   // mutated to `if (false)`
```

No suite failed. The refusal is unit-covered, but every real-server governed
scenario we have makes verified progress in every window, so a gate that never
refuses behaves identically to one that refuses correctly.

What that means precisely: we have proved that verified progress AUTHORIZES a
second request (`governed-verified-progress-lifecycle-postgres-test`), and we
have not proved through the real scheduler that its ABSENCE withholds one. The
two are different claims, and only the first is currently defended end to end.

**What would close it.** A real-server scenario whose first governed response
commits a receipt WITHOUT satisfying any admitted fact, under
`maximumConsecutiveNoProgressWindows = 1`, asserting that no second runtime
budget reservation, no second economic reservation and no second transport call
occur, and that a `verified_progress_exhausted` block persists. The same
scenario covers the receipt-committed/evidence-absent integrity refusal, which
is likewise only unit-covered today.

**Why it is not merely theoretical.** `verified_progress_exhausted` is a
persisted, operator-visible reason for stopping someone's work. If the gate
silently stopped refusing, Runs that should stop would keep spending, and the
first evidence would be a bill rather than a failing test.
