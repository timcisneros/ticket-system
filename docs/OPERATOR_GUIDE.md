...
warning flags, recent failures/refusals, process-local event-append admission/pressure, and links to
the relevant pages. `GET /api/runtime/status` and `oquery runtime-status` expose the same live append
pressure for runtime diagnostics. These metrics do not bound total event-log growth. Reading these
surfaces writes nothing and creates no summary ledger. See `docs/OPERATIONAL_TRANSPARENCY.md`.
