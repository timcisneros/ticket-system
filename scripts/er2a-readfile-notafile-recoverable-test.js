...
const DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), 'er2a-readfile-notafile-data-'));
const WORKSPACE_ROOT = createTempWorkspaceRoot('er2a-readfile');
...
function createFakeOpenAIPreload() {
  const preloadPath = path.join(os.tmpdir(), `er2a-openai-${process.pid}-${Date.now()}.js`);
  const source = `
function okResponse(plan) {
  return {
    ok: true, status: 200,
    headers: new Map([['x-request-id', 'fake-er2a']]),
    async text() {
...
global.fetch = async function(url, options = {}) {
  ...
  if (combined.includes('er2a-notafile')) {
    counters.notafile = (counters.notafile || 0) + 1;
    if (counters.notafile === 1) {
      return okResponse({
        message: 'Reading a directory path.',
        actions: [{ operation: 'readFile', args: { path: 'my-folder' } }],
        complete: false
      });
    }
    return okResponse({
...
async function main() {
  ...
  try {
    ...
    // ── Test 1: readFile on directory does NOT terminate the run ──
    const ticket1 = await createTicket(cookie, agent, `er2a-notafile ${STAMP}`);
    ...
    console.log('  ✓ readfile-notafile-recoverable: error does not terminate run, model retries with listDirectory');

    console.log(JSON.stringify({ er2aReadfileNotafileRecoverable: true }));
  } finally {
    if (server) {
      server.kill();
