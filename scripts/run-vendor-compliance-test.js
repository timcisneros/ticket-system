...
async function main() {
  console.log('Vendor Compliance Existing Workflow Test');
  console.log('========================================\n');
  ...
  console.log('\n[0b] Cleaning stale output artifacts...');
  for (const f of ['vendor-decision-register.csv', 'compliance-review.md']) {
    const fp = path.join(WORKSPACE_ROOT, 'vendors', f);
  ...
  console.log('\n[3] Creating vendor-compliance workflow ticket (basePath=vendors)...');
  const ticketId = await createWorkflowTicket(
    'Classify 40 vendor compliance packets from vendors/incoming/',
    'vendor-compliance',
    { basePath: 'vendors' }
  );
  ...
  for (const f of ['vendors/vendor-decision-register.csv', 'vendors/compliance-review.md']) {
