...
function reconstructStateMachine(events) {
  ...
  const validTransitions = {
    ...
    completed: ['completed'], failed: ['failed'], interrupted: ['interrupted'], terminalized: ['terminalized']
  };
  const stateEventMap = {
...
function reconstructRun(data, runId) {
  ...
  const hasExecutionCompleted = runEvents.some(e => e.type === 'run.execution_completed') ||
    runEvents.some(e => ['run.completed', 'run.failed', 'run.interrupted'].includes(e.type));
