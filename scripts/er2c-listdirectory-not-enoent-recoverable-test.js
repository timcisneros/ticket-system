...
const DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), 'er2c-listdir-not-enoent-data-'));
const WORKSPACE_ROOT = createTempWorkspaceRoot('er2c-listdir');
...
function createFakeOpenAIPreload() {
  const preloadPath = path.join(os.tmpdir(), `er2c-openai-${process.pid}-${Date.now()}.js`);
  const source = `
function okResponse(plan) {
  return {
    ok: true, status: 200,
    headers: new Map([['x-request-id', 'fake-er2c']]),
    async text() {
...
global.fetch = async function(url, options = {}) {
  ...
  if (combined.includes('er2c-notadir')) {
    counters.notadir = (counters.notadir || 0) + 1;
    if (counters.notadir === 1) {
      return okResponse({
        message: 'Listing a path that is a file, not a directory.',
        actions: [{ operation: 'listDirectory', args: { path: 'not-a-dir.txt' } }],
        complete: false
      });
    }
    return okResponse({
      message: 'Listing root directory after not-a-directory error.',
...
async function main() {
  fs.writeFileSync(path.join(WORKSPACE_ROOT, 'not-a-dir.txt'), 'this is a file, not a directory\n');
  ...
  try {
    ...
    // ── Test 1: listDirectory on a file does NOT terminate the run ──
    const ticket1 = await createTicket(cookie, agent, `er2c-notadir ${STAMP}`);
    ...
    const listOps = run1.replaySnapshot.workspaceOperations.filter(
      op => op.operation && op.operation.operation === 'listDirectory' && op.operation.args.path === 'not-a-dir.txt'
    );
    assert(listOps.length === 1, `Should have exactly one listDirectory on not-a-dir.txt, got ${listOps.length}`);
    assert(listOps[0].error != null, `listDirectory(not-a-dir.txt) should have error, got: ${JSON.stringify(listOps[0])}`);
    assert(listOps[0].blocked === false, `blocked should be false (workspace_error), got ${listOps[0].blocked}`);
    ...
    console.log('  ✓ listdirectory-not-enoent-recoverable: ENOTDIR does not terminate run, model retries with root list');

    console.log(JSON.stringify({ er2cListdirectoryNotEnoentRecoverable: true }));
  } finally {
    if (server) {
      server.kill();
